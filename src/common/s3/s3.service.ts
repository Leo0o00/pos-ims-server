...
import { getSignedUrl } from '@aws-sdk/s3-request-presigner'; // const SIGNED_URL_EXPIRATION_TIME = 300; // 5 minutes

// const SIGNED_URL_EXPIRATION_TIME = 300; // 5 minutes
const SIGNED_URL_EXPIRATION_TIME = 28800; // 8 hours
...
export class S3Service {
  ...
  constructor(
    ...
    this.secretAccess_key = this.configService.get<string>(
    ...
    if (
      ...
      !this.accessKey ||
      !this.bucketRegion ||
      !this.secretAccess_key
    )
      throw new Error('AWS S3 service credentials are not configured.');
  ...
  async uploadFile(fileBuffer: Buffer, fileName: string, mimetype: string) {
    ...
    return uploadResponse.$metadata.httpStatusCode === 200;
  }
